...
# Универсальная функция для набора тестов (ищем неподвижную точку или цикл)
...
def printTable(magicNumbers, cycle_info, base_max, digits_max):
    ...
    for items in cycle_info:
        ...
        elif table[len(items[0][0]) - 2][items[1] - 2][2] == 0 and table[len(items[0][0]) - 2][items[1] - 2][0] != "NONE":
    ...
    #Попробуем воспользоваться библиотекой пандас для построения таблицы
    k = [str(i) + " base" for i in range(2, base_max + 1)]
    j = [str(i) + " digits" for i in range(2, digits_max + 1)]
    
    output = pandas.DataFrame(table, j, k)
...
def main():
    ...
    radix = 2
    base_max = 10
    digits_max = 5 #Для 6 вычисления получаются достаточно долгими
